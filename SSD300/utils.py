...
# DIOR 20-class label map
# background=0, classes=1..20  (order matches dior_dataset.py CLASSES)
dior_labels = (
...
label_map = {name: idx + 1 for idx, name in enumerate(dior_labels)}
label_map['background'] = 0
rev_label_map = {v: k for k, v in label_map.items()}

# 21 distinct colors for visualization (index 0 = background, 1-20 = DIOR classes)
distinct_colors = [
...
label_color_map = {k: distinct_colors[v] for k, v in label_map.items()}
...
def gcxgcy_to_cxcy(gcxgcy, priors_cxcy):
    ...
    return torch.cat([gcxgcy[:, :2] * priors_cxcy[:, 2:] / 10 + priors_cxcy[:, :2],  # c_x, c_y
                      torch.exp(gcxgcy[:, 2:].clamp(max=10.) / 5) * priors_cxcy[:, 2:]], 1)  # w, h  — clamp prevents exp() overflow to inf early in training
...
def flip(image, boxes):
    ...
    # Flip boxes — clone() so the caller's tensor is not mutated in-place
...
def vflip(image, boxes):
    """
    Flip image vertically. Important for satellite imagery where objects
    appear at all orientations — vertical flip is as valid as horizontal.
    ...
    new_boxes = boxes.clone()
    orig_ymin = boxes[:, 1].clone()
    orig_ymax = boxes[:, 3].clone()
    new_boxes[:, 1] = image.height - orig_ymax - 1
    new_boxes[:, 3] = image.height - orig_ymin - 1
...
def transform(image, boxes, labels, difficulties, split):
    ...
    # ImageNet mean/std — matches the distribution ResNet50 was pretrained on
    ...
    if split == 'TRAIN':
        ...
        # Flip image vertically with a 50% chance — satellite images have no
        # canonical "up", so vertical flip is as valid as horizontal
        if random.random() < 0.5:
